...
def write_csv(seqs, path):
    with open(path, "w") as output:
        for name, seq in seqs.items():
            output.write("{},1\n".format(seq))

def padding_seqs(seqs, seqwin, length=None, pad_value=None):
    length = length or seqwin
...
def decode(seqs, seqwin, table):
    decode_seqs = []
    for seq in seqs:
        seq = seq.reshape(seqwin, -1)
        decode_seq = ""
...
def generate_seqs(net, seqwin, table, noise, num_eval):
    generated = net(noise)
    decoded_seqs = decode(generated, seqwin, table)
    seqs = {}
    for i, decoded_seq in enumerate(decoded_seqs):
        #seq = [aa for aa in decoded_seq if aa != "X"]
        seq = [aa for aa in decoded_seq]
