...
gen_test_csv=${main_path}/data/dataset/gen_independent_test/gen_independent_test.csv
...
do
    ...
    do
        ...
        if echo "${encode_method}" | grep -q "W2V"; then
            ...
            python ml_train_test_47.py  --intrain ${train_path} --test ${test_csv} --gen_test ${gen_test_csv} --outpath ${result_path} --machine ${machine_method}  --encode ${encode_method} --kfold ${kfold} --seqwin ${seqwin} --kmer ${kmer} --w2vmodel ${w2v_model} --size ${size} --epochs ${epochs} --sg ${sg} --window ${window} --esm2 ${esm2_dict}        
