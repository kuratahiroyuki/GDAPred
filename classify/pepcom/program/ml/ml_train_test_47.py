...
def pad_input_csv_2(filename, seqwin, index_col = None):
    df1 = pd.read_csv(filename, delimiter=',', index_col = index_col)
    print(df1)
    exit()
    seq = df1.loc[:,'seq'].tolist()
...
if __name__=="__main__":
    ...
    parser.add_argument('--test', help='Path')
    parser.add_argument('--gen_test', help='Path')    
    ...
    gen_test_file = parser.parse_args().gen_test
    ...
    for i in range(1, kfold+1):
        ...
        gen_test_dataset = pad_input_csv(gen_test_file, seqwin, index_col = None)

        train_seq = train_dataset['seq'].tolist()
        val_seq = val_dataset['seq'].tolist()
        test_seq = test_dataset['seq'].tolist()
        gen_test_seq = gen_test_dataset['seq'].tolist()
        
        #print(f"{len(gen_test_seq)=}") ### 1000
        ...
        if encode_method == 'W2V':
            ...
            gen_test_X = emb_seq_w2v(gen_test_seq, w2v_model, kmer, size)
            ...
            gen_test_X = emb_seq_BE(gen_test_seq, aa_dict, kmer)
            ...
            gen_test_X = emb_seq_ESM2(gen_test_seq, seq2esm2_dict)  
            ...
            gen_test_seq =gen_test_dataset.values.tolist()
            ...
            if encode_method == 'AAC':
                ...
                gen_test_X = np.array(AAC(gen_test_seq, **kw), dtype=float)
                ...
                gen_test_X = np.array(TPC(gen_test_seq, **kw), dtype=float)
                ...
                gen_test_X = np.array(DPC(gen_test_seq, **kw), dtype=float)
                ...
                gen_test_X = np.array(EAAC(gen_test_seq, **kw), dtype=float)
                ...
                gen_test_X = np.array(CTriad(gen_test_seq, **kw), dtype=float)
                ...
                gen_test_X = np.array(GAAC(gen_test_seq, **kw), dtype=float)
                ...
                gen_test_X = np.array(GDPC(gen_test_seq, **kw), dtype=float)
                ...
                gen_test_X = np.array(GTPC(gen_test_seq, **kw), dtype=float)
                ...
                gen_test_X = np.array(CTDC(gen_test_seq, **kw), dtype=float)
                ...
                gen_test_X = np.array(CTDD(gen_test_seq, **kw), dtype=float)
                ...
                gen_test_X = np.array(CTDT(gen_test_seq, **kw), dtype=float)
                ...
                gen_test_X = np.array(PAAC(gen_test_seq, **kw), dtype=float)      
                ...
                gen_test_X = np.array(CKSAAP(gen_test_seq, **kw), dtype=float)
                ...
                gen_test_X = np.array(CKSAAGP(gen_test_seq, **kw), dtype=float)
                ...
                gen_test_X = np.array(AAINDEX(gen_test_seq, **kw), dtype=float)
                ...
                gen_test_X = np.array(BLOSUM62(gen_test_seq, **kw), dtype=float)
                ...
                gen_test_X = np.array(ZSCALE(gen_test_seq, **kw), dtype=float)
        ...
        gen_test_y = gen_test_dataset['label'].to_numpy()
        ...
        gen_test_result = np.zeros((len(gen_test_y), 2))
        gen_test_result[:,1] = gen_test_y   #score:one of two, label
        ...
        if len(test_y) != 0:
            if machine_method == 'LGBM':
                ...
                gen_test_result[:, 0] = clf.predict(gen_test_X, num_iteration=clf.best_iteration)
            elif machine_method == 'XGB': 
                test_result[:, 0] = clf.predict(xgb.DMatrix(test_X))
                gen_test_result[:, 0] = clf.predict(xgb.DMatrix(gen_test_X))                
            else:
                test_result[:, 0] = clf.predict_proba(test_X)[:,1]
                gen_test_result[:, 0] = clf.predict_proba(gen_test_X)[:,1]         
        #CV  
        ...
        gen_test_output = pd.DataFrame(gen_test_result,  columns=['prob', 'label'] )
        gen_test_output.to_csv(out_path  + "/" + str(i) + "/gen_test_roc.csv")  #prob, label   
